...
class KarAmoozList(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # تبدیل به لیست Python
        karamooz_list = list(queryset.order_by('-id'))  # ابتدا مرتب بر اساس id نزولی

        # پیدا کردن شخص موردنظر
        for i, k in enumerate(karamooz_list):
            if k.name == "محمدرضا" and k.lastname == "اسماعیلی":
                # حذف از جای خودش و اضافه کردن اول لیست
                karamooz_list.insert(0, karamooz_list.pop(i))
                break  # فقط اولین مورد

        return karamooz_list
